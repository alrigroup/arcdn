...
static void trim_copy(char *out, const char *in, int in_len, int out_size) {
  int s = 0, e = in_len;
  while (s < e &&
         (in[s] == ' ' || in[s] == '\t' || in[s] == '\r' || in[s] == '\n'))
    s++;
  while (e > s && (in[e - 1] == ' ' || in[e - 1] == '\t' || in[e - 1] == '\r' ||
                   in[e - 1] == '\n'))
    e--;
  int n = e - s;
  if (n >= out_size)
...
static int get_cfg_path(void) {
  ...
  if (home_os_get_exe_dir(exedir, sizeof(exedir)) == 0 && exedir[0]) {
    ...
    snprintf(g_cfg_path, sizeof(g_cfg_path),
             "%s\\..\\..\\storage\\cdn\\cdn.cfg", exedir);
#else
    snprintf(g_cfg_path, sizeof(g_cfg_path), "%s/../../storage/cdn/cdn.cfg",
             exedir);
#endif
    return 0;
  }
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (g_entry_count < MAX_ENTRIES) {
      snprintf(g_entries[g_entry_count].path,
               sizeof(g_entries[g_entry_count].path), "%s", key);
      snprintf(g_entries[g_entry_count].file,
               sizeof(g_entries[g_entry_count].file), "%s", val);
      g_entry_count++;
    }
  }
  fclose(f);
...
static int ipc_send_frame(int fd, unsigned char type, const char *data,
                          int len) {
...
static int register_route(int fd, const char *prefix, const char *host,
  ...
  int len =
      snprintf(payload, sizeof(payload),
               ...
               APP_NAME, prefix, "GET", host, mode, GATEWAY_HOST, server_port);
  if (len <= 0 || len >= (int)sizeof(payload))
    return -1;

  if (ipc_send_frame(fd, 1, payload, len + 1) != 0)
    return -1;

  unsigned char type = 0;
  char ack[128];
  if (ipc_recv_frame(fd, &type, ack, sizeof(ack)) != 0)
    return -1;
...
static int cmd_add(const char *args, char *resp, int size) {
  ...
  if (!is_valid_route_path(path) || !file[0]) {
    return snprintf(
        resp, size,
        "usage: add <path> <file>  (path must start with /, no '..' or '%%')");
  }
  if (strlen(file) >= 1536) {
  ...
  if (file_get_size(abs, &sz) != 0) {
    return snprintf(resp, size, "file not found or not a regular file: %s",
                    abs);
  }
  ...
  if (save_rc != 0) {
    return snprintf(resp, size,
                    "add ok (in memory) but failed to persist cdn.cfg");
  }
  return snprintf(resp, size, "%s %s -> %s (%lld bytes)",
                  added ? "added" : "updated", path, abs, sz);
}
...
static int cmd_del(const char *args, char *resp, int size) {
  ...
  if (!found)
    return snprintf(resp, size, "not found: %s", path);
  if (save_rc != 0)
    return snprintf(resp, size,
                    "deleted (in memory) but failed to persist cdn.cfg");
...
static void handle_query(int fd, const char *q, int len) {
  ...
  if (strncmp(q, "help", 4) == 0 || strncmp(q, "--help", 6) == 0 ||
      strncmp(q, "-h", 2) == 0 || q[0] == '\0') {
    rlen = snprintf(
        resp, sizeof(resp),
        "CDN Delivery Engine & Media Streamer v0.2.0 (Self-Registered)\n\n"
        "Supported Commands:\n"
        "  status                - View server port, active entries and worker "
        "status\n"
        "  routes                - List all active static URL routes and "
        "mappings\n"
        "  list                  - Display detailed list of hosted assets and "
        "sizes\n"
        "  add <path> <file>     - Register a new static file into CDN "
        "storage\n"
        "  del <path>            - Delete a static asset from memory and "
        "cdn.cfg\n"
    ...
    rlen = snprintf(resp, sizeof(resp), "%s RUNNING port=%d entries=%d",
                    APP_NAME, server_port, g_entry_count);
...
static void query_loop(int fd) {
  ...
  while (1) {
    ...
    if (ipc_send_frame(fd, IPC_HEARTBEAT, NULL, 0) < 0)
      break;
  }
...
static void send_plain(int c, int status, const char *status_text,
  ...
  if (len > 0)
    send_all(c, header, len);
  if (body_len > 0)
    send_all(c, body, body_len);
}

static void send_404(int c) {
  send_plain(c, 404, "Not Found", "application/json",
             "{\"error\":\"Not found\"}", -1);
}
...
static const char *find_header_value(const char *buf, int total,
                                     const char *name) {
...
static int parse_range(const char *hdr, long long size, long long *start,
                       long long *end) {
  if (!hdr)
    return 0;
  char local[96];
...
static void serve_file(int c, const char *file, const char *range_hdr,
                       int is_head) {
  long long size;
  ...
  int hlen = snprintf(header, sizeof(header),
                      ...
                      "Connection: close\r\n\r\n",
                      status, status_text, get_mime(file), clen, range_extra,
                      g_cache_ttl);
  if (hlen > 0)
  ...
  if (!f)
    return;
  if (start > 0)
    file_seek_set(f, start);

  char buf[32768];
  long long remaining = clen;
  while (remaining > 0) {
    int want =
        remaining > (long long)sizeof(buf) ? (int)sizeof(buf) : (int)remaining;
...
static void serve(int c, const char *path, const char *range_hdr, int is_head) {
  ...
  if (strcmp(clean, "/") == 0 || clean[0] == '\0') {
    ...
    int blen = snprintf(
        body, sizeof(body),
        "{\"message\":\"ALRI CDN\",\"status\":\"active\",\"entries\":%d}",
        g_entry_count);
  ...
  if (!is_valid_route_path(clean)) {
    send_plain(c, 400, "Bad Request", "application/json",
  ...
  char file[1536] = {0};
  if (e)
    snprintf(file, sizeof(file), "%s", e->file);
  ar_mutex_unlock(g_mutex);
  ...
  if (!file[0]) {
    ...
    if (strncmp(clean, "/avatars/pfp_", 13) == 0) {
      ...
      int valid = 1;
      for (int i = 0; fn[i]; i++) {
        if (!isalnum((unsigned char)fn[i]) && fn[i] != '.' && fn[i] != '_' &&
            fn[i] != '-') {
          valid = 0;
          break;
        }
      ...
      if (valid && strlen(fn) > 4) {
        ...
        snprintf(cand3, sizeof(cand3), "arcore/storage/enterprise/avatars/%s",
                 fn);
        snprintf(cand4, sizeof(cand4), "arcore/storage/arenterprise/avatars/%s",
                 fn);
        snprintf(cand5, sizeof(cand5), "../../storage/enterprise/avatars/%s",
                 fn);
    ...
    if (!file[0] && strncmp(clean, "/media/", 7) == 0) {
      ...
      if (valid && strlen(fn) > 3) {
        ...
        snprintf(cand3, sizeof(cand3), "arcore/storage/enterprise/media/%s",
                 fn);
        snprintf(cand4, sizeof(cand4), "arcore/storage/arenterprise/media/%s",
                 fn);
